...
class CPULC(CPU):
    ...
    def op(self, sig):
        ...
        print(sig, self.bus, bin2int(self.PC))
        print([bin2int(self.R.get(int2bin(_, 3))) for _ in range(8)])
        return BRK, HLT

    def pre(self):
        print("==== <pre> ====")
        self.op(self.ISA.names["op0"])
        self.op(self.ISA.names["op1"])
        print("==== </pre> ====")
